    ...
    public static void main(String[] args) {
        ...
        2. 执行 add()
         ...
         final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
                ...
                //(2)判断p 是否为null
                //(2.1) 如果p 为null, 表示还没有存放元素, 就创建一个Node (key="java",value=PRESENT)，key就是真正要传进去的东西，value的PRESENT永远是一个固定的默认值
                //(2.2) 就放在该位置 tab[i] = newNode(hash, key, value, null)
                这里说的是经过计算在table中找到了一个位置来放元素，如果这个位置没有元素，那么就创建一个新的节点，把这个值放入
                if ((p = tab[i = (n - 1) & hash]) == null)
                    ...
                    // 就不能加入!!!
                    // 这里的p就是现在指向java的Node对象，key就是p指向的Node节点的key，k就是准备加入的key(传入的字符串)
                    if (p.hash == hash &&
                    ...
                    //如果是一颗红黑树，就调用 putTreeVal , 来进行添加
                        ...
                        e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value);
                          ...
                          //    注意在把元素添加到链表后，立即判断 该链表是否已经达到8个结点(也就是TREEIFY_THRESHOLD), 就调用 treeifyBin() 对当前这个链表进行树化(转成红黑树)
                          //    注意，在转成红黑树时，要判断条件如下
